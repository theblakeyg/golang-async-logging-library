...
func (al Alog) Start() {
	//this was what I had originally but that is not the right syntax for this implementation
...
loop:
	for {
		...
		case msg := <-al.msgCh:
			...
			go al.write(msg, wg)
		case <-al.shutdownCh:
			wg.Wait() //allows the channel to be drained before it shuts it down
			go al.shutdown()
			break loop
		}
	}
...
func (al Alog) write(msg string, wg *sync.WaitGroup) {
	al.m.Lock()
	defer al.m.Unlock() //allows to unlock mutex after a panic
